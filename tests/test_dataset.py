...
def test_dataset_loading(img_size: int = 256):
    ...
    train_dataset = BrainTumorDataset(data_folder=raw_data_dir, train=True, img_preprocess= ArrayPreprocessing(img_size=img_size), img_transforms = TensorsPreprocessing())
    test_dataset = BrainTumorDataset(data_folder=raw_data_dir, train=False, img_preprocess= ArrayPreprocessing(img_size=img_size), img_transforms = TensorsPreprocessing())
...
def test_dataset_statistics():
    ...
    assert train_label_distribution == expected_train_counts, f"Expected training counts {expected_train_counts}, got {train_label_distribution}"
    assert test_label_distribution == expected_test_counts, f"Expected testing counts {expected_test_counts}, got {test_label_distribution}"