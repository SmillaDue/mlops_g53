...
data_transforms = Compose([lambda img: cv2.imread(str(img)),
                    crop_img,
                    ToTensor(),
...
def load_data(raw_data_dir: str):
    ...
    image_files = [
        [os.path.join(train_data_dir, class_names[i], x) for x in os.listdir(os.path.join(train_data_dir, class_names[i]))]
